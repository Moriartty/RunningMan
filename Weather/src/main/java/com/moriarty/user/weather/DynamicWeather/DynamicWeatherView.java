package com.moriarty.user.weather.DynamicWeather;
...
import com.moriarty.user.weather.DynamicWeather.BaseDrawer.Type;
...
public class DynamicWeatherView extends SurfaceView implements SurfaceHolder.Callback {
	...
	private Type curType = Type.CLEAR_D;
	...
	private class DrawThread extends Thread {
		...
		public void run() {
			while (true) {
				...
				synchronized (this) {
					//由于weatherActivity退出后并不会立即调用的onDestroy()
					//所以surface并不会立即销毁==null,所以这里会一直等待
					//导致界面不会及时退出,所以改为在weatherActivity中监听回退按钮事件
					//直接调用onPause(),onDestroy()
